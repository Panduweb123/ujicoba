...
export default function Keranjang() {
  ...
  // STATE BARU: Untuk menyimpan inputan nama pelanggan
  ...
  const handleCheckoutTunai = async () => {
    // Validasi: Pastikan pelanggan sudah mengisi nama
    ...
    try {
      ...
      // Simpan ke tabel pesanans (SEKARANG MENYERTAKAN NAMA PELANGGAN)
        ...
        .insert([
          {
            ...
            nama_pelanggan: namaPelanggan, // <--- Data nama dimasukkan ke database
      ...
      clearCart();
      localStorage.removeItem("meja_id_aktif");
      localStorage.removeItem("nama_meja_aktif");
  ...
  return (
    <div className="max-w-3xl mx-auto bg-slate-50 min-h-screen pb-[320px]">
      ...
      <div className="fixed bottom-0 left-0 right-0 max-w-3xl mx-auto bg-white p-6 border-t border-gray-200 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] rounded-t-3xl z-50">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <button className="bg-blue-600/10 text-blue-700 border-2 border-blue-600 py-4 rounded-xl font-bold text-lg hover:bg-blue-600 hover:text-white transition-colors">
            Bayar QRIS
          </button>
          ...
          >
            {isProcessing ? "Memproses..." : "Bayar Tunai Sekarang"}
          </button>
        </div>
      </div>
    </div>
